...
## 变更记录
- v1.0 初版。
- v1.1 (2026-07-16) H2 范围调整（不降级目标）：公开机读逐日射电时序仅 GX 339-4 (Corbel+13 VizieR) 可得；J1820/GX339-4 已发表 τ 值作为独立文献点纳入比较。H2 检验改为"GX 339-4 多爆发自测 τ + 跨源文献 τ"的 MAD 形成时标标度分析；曾尝试 Zenodo/GitHub/RATAN/AMI 公开渠道获取 J1348/J1820/H1743 射电时序未果（记录于 analysis_log.md）。
- v1.2 (2026-07-16) H3 实现细化：φ_jet 反演采用 3 组喷流功率标定敏感性传播；硬态窗 L_bol/L_Edd∈[1e-4,0.05]。新增否决窗口表处理场源污染与非典型源。
