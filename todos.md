...
- [ ] 转变光度换算 L/L_Edd；与 Vahdat Motlagh+19、WATCHDOG 已发表值交叉验证（偏差>2σ 者排查）
- [ ] 阶段小结写入 docs/analysis_log.md

## Phase 4 射电数据与 τ 测量 (H2)
- [ ] 整理 XRB-LrLx_pub BH 表；逐源列出可用射电监测时序及文献
- [ ] 收集逐日/逐周射电监测光变（VizieR/论文表：Corbel+13、Bright+20、Russell+19、Carotenuto+21、Tetarenko+17 等）
- [ ] code/ccf.py：ICCF + DCF + FR/RSS MC 误差；单元测试（模拟已知延迟光变恢复 τ）
- [ ] 试点：J1820 检验能否复现 τ≈8 d（对照 You+23）、GX339-4 decay τ≈8 d（对照 2605.19473）→ 不一致则迭代方法
- [ ] 全样本逐爆发 τ 测量（rise/decay 分段），质量分级（A/B/上限）
- [ ] ICCF/DCF 诊断图集目检；τ 结果表 output/results/tau_table.csv

## Phase 5 φ_jet 与 MAD 饱和度 (H3)
- [ ] code/phi_jet.py：P_jet(L_R) 多标定、Ṁ(L_X)、φ_jet 及 MC 误差传播
- [ ] 逐爆发 φ_jet(t) 轨迹图 + 硬态峰值 φ_max,obs 表
- [ ] 与 φ_MAD≈50 对比；标定敏感性矩阵
- [ ] 目检全部 φ 轨迹图，异常源排查（距离/自旋输入错误等）

## Phase 6 统计推断与假设检验
- [ ] H1：L_trans/L_Edd vs M(R_s) — Spearman+偏相关+贝叶斯回归斜率后验；rise/decay 分开
- [ ] H2：τ vs M、τ vs L_peak/L_Edd、τ vs P_orb 标度回归（含上限生存分析）
- [ ] H3：φ_max,obs vs 回滞幅度/转变光度残差 偏相关（控制 R_s、距离）
- [ ] 稳健性：留一法、距离 MC、标定敏感性；全部写入 output/results/
- [ ] corner 图与关键关系图目检迭代至发表级

## Phase 7 可视化与成果
- [ ] 发表级图集（统一样式）：样本光变图集、HID 图集、L_trans–M、τ–M、φ 轨迹、Lr–Lx(φ 着色)、后验 corner、敏感性热图
- [ ] 逐张 read 目检：清晰度、标注、配色、误差棒完整性 → 迭代
- [ ] docs/results_summary.md：方法、结果、与理论对照、创新点、可发表结论
- [ ] 复查：无数据编造、所有数值可溯源（脚本+日志+数据文件三链路）
- [ ] 更新 literature_review/proposal 变更记录

## Phase 8 交付
- [ ] 清理无用中间文件，检查磁盘
- [ ] README.md（仓库结构、复现步骤）
- [ ] 全部内容 push 到 GitHub；核验远端文件完整
- [ ] 最终自查 todos 全部闭环
